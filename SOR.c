/*Ejercicio práctica 1, Mètodes Numèrics 2, SOR*/
...
int sor(int,double,double**, double*, double*);
...
int main(void){
	...
	int iter = sor(n,w,x, radi_espectral, error_sistema);
	...
	printf("\nNombre d'iterats per la convergència de SOR : %d \n",iter);
...
int sor(int n, double w,double **x, double *radi_espectral, double *error_sistema){
	int i,j, i2, j2;
	double **b;
	int n_quadrat = n*n;
	...
	*/
	...
	int iter = 0;
	double xPrev, sum, max;
	double dif, r, error_ant;
	...
	while(max>TOL && iter<MAX_ITER){
		...
		for(i=0;i<n_quadrat;i++){
			...
			*/
	        ...
	        xPrev = x[i2][j2]; //guardem el valor previ pel criteri d'aturada

	        r = (b[i2][j2]-sum) / A_matrix(i,i,n);
	        x[i2][j2] = x[i2][j2] + w*r;


	        //CALCULEM LA NORMA DEL MÀXIM DE LA DIFERÈNCIA PEL CRITERI D'ATURADA
	        dif = fabs(x[i2][j2] - xPrev);
			//actualitzem si trobem un valor major
	        max = maxim(max,dif); //max(max, dif)
...
double g_function(int i, int j, double h, int n){
	...
	else if(i==n+1){ //g(n+1,y) que sobre omega és g(1,y)= y⁴ - y²
		return sin(y)-2*y*y*cos(1);
	}
	else if(j==n+1){ //g(x,n+1) que sobre omega és g(x,1)= x - x³
...
void crear_documento(double **x, int n){
	FILE *file=fopen("resultadosSOR", "w");
