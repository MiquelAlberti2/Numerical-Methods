/*Ejercicio práctica 1, Mètodes Numèrics 2, Gauss-Seidel*/
...
int gaussseidel(int,double**, double*, double*);
...
int main(void){
	...
	int iter = gaussseidel(n,x, radi_espectral, error_sistema);
	...
	printf("\nNombre d'iterats per la convergència de GS : %d \n",iter);
...
int gaussseidel(int n,double **x, double *radi_espectral, double *error_sistema){
	int i,j, i2, j2;
	double **b;
	int n_quadrat = n*n;
...
double g_function(int i, int j, double h, int n){
	...
	else if(i==n+1){ //g(n+1,y) que sobre omega és g(1,y)
		return sin(y)-2*y*y*cos(1);
	}
	else if(j==n+1){ //g(x,n+1) que sobre omega és g(x,1)
...
void crear_documento(double **x, int n){
	FILE *file=fopen("resultadosGS", "w");
